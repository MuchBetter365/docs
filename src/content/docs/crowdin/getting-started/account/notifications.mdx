...
<Steps>
  ...
  1. Paste the following payload in the **Payload** field:
      ...
      ```
      This payload uses the Microsoft Teams Adaptive Card message format, where `type` must be `message`, `attachments` contains the card objects, `contentType` must be `application/vnd.microsoft.card.adaptive`, and `content` contains the Adaptive Card JSON.
